...
[fith_d5x] = tramo5(x, ch_d5x);
% obtengo los coeficientes de las gaussianas
c_d5x = coeffvalues(fith_d5x);
c_d5x = reshape(c_d5x, 3, 5);
b_d5x = sort(c_d5x(2,:));
sig_d5x = sort(c_d5x(3,:));

dist1 = b_d5x(3) - (b_d5x(1) + b_d5x(2))/2;
dist2 = b_d5x(4) - b_d5x(3);
...
dist1 = b_d6x(3) - b_d6x(2);
dist2 = b_d6x(4) - b_d6x(3);
...
ch2 = squeeze(Ic(41,:));%cortes horizontales
[fith, gofh] = tramo7(x, ch2);
% obtengo los coeficientes de las gaussianas
...
%corte de la imagen
Ic = imcrop(J, [xmin ymin ancho alto]);%[xmin ymin width height]%
ch2 = squeeze(Ic(45,:));%cortes horizontales
[fith, gofh] = tramo8(x, ch2);
% obtengo los coeficientes de las gaussianas
cesc1 = coeffvalues(fith);
besc1 = reshape(cesc1, 3, 4);
b_esc1 = sort(besc1(2,:));
...
ejey_um = rp*vey2;%eje x transformado por escal2a

Vy_m1= ejey_px(1:11);%eje -x
Vy_m2= ejey_px(11:21);%eje +x
% 
...
% figure; plot(pv2, resfv, '*r')
% %h = plotResiduals(fv,'fitted');
% title('Residuos 1 mitad Y');
% xlabel('Posición');
% ylabel('Residuos');  
% 
% sv = LinearModel.fit(pv2, Vy_m2); 
% figure; plot(sv)
